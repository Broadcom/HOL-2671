# labstartup.py - version 1.6.3 - 17-March 2023
...
# this is default value (discuss with Core Team before changing)
lsf.max_minutes_before_fail = 30
...
# moved to /home/holuser/desktop-hol/lmcstart.sh
...
# lsf.labfail('Test delete enpoint')
##############################################################################
#  Main LabStartup
##############################################################################
...
if os.path.exists('/tmp/lupdwired') and lsf.manager == True:
...
if not lsf.labcheck:
    ...
    try:
        ...
        lsf.labfail('git pull failed')
else:
    lsf.write_output("labchecking, so we'll skip git pull")

# try:
#     process = subprocess.run(["/usr/bin/git", "switch", "2437"], cwd='/hol/hol-2x37', check=True, capture_output=True, text=True)
#     lsf.write_output(process)
#     lsf.write_output(process.stdout)
# except Exception as e:
#     lsf.write_output(e)
#     lsf.write_output(process.stdout)
#     lsf.write_output(process.stderr)
...
##############################################################################
#  Lab Startup - STEP #pre6 (Nicks tweak script)
##############################################################################
lsf.write_vpodprogress('Running adjustomatic', 'GOOD-5')
...
labcheck = False
if not labcheck and lsf.manager == True:
    lsf.write_output(
